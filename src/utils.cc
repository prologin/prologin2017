...
void print_workbenches(const GameState& game_state, std::ostream& stream)
{
    ...
    stream << std::setw(4) << current_player.get_score() << color_reset
           << color_bold << "     " << color_reset << bgcolor(7) << fgcolor(0)
           << "adv";
    std::fill_n(std::ostream_iterator<char>(stream), TAILLE_ETABLI - 5, ' ');
    ...
    for (uint8_t y = 0; y < TAILLE_ETABLI; ++y)
    {
        ...
        for (uint8_t x = 0; x < TAILLE_ETABLI; ++x)
            stream << bgcolor(
                          game_state.get_cell_type({y, x}, current_player_id))
                   << " ";
        ...
        {
            stream << "  " << bgcolor(current_sample.element1) << " "
                   << bgcolor(current_sample.element2) << " " << color_reset
                   << " ";
        }
        else
            stream << "     ";
    ...
    const std::string current_player_name =
        current_player.get_name().substr(0, TAILLE_ETABLI + 2);
    const std::string opponent_name =
        opponent.get_name().substr(0, TAILLE_ETABLI + 2);

    stream << bgcolor(7) << fgcolor(0) << std::setw(TAILLE_ETABLI + 2)
           << current_player_name;
    stream << color_reset << "     ";
    stream << bgcolor(7) << fgcolor(0) << std::setw(TAILLE_ETABLI + 2)
           << opponent_name;
